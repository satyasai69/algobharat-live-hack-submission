...
By default the template creates a single `HelloWorld` contract under algorand_challenge folder in the `smart_contracts` directory. To add a new contract:
...
#### Setting up GitHub for CI/CD workflow and TestNet deployment
  ...
  2. Decide what values you want to use for the `allowUpdate` and `allowDelete` parameters specified in [`deploy-config.ts`](./smart_contracts/algorand_challenge/deploy-config.ts).
